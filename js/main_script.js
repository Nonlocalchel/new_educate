...
//кнопка помощь
try{
	let buttonSupport=document.querySelector('.btn__support');
...
try{
	let buttonLogin=document.querySelector('.login__link');
...
try{
	...
	for(key of linkList){
		...
		if(location==key.href && key.getAttribute('class')!="logo_img"){ //тут красим основную навигацию
			if(key.href.slice(22,)!="index.html"){
...
try{
	...
	window.addEventListener('scroll',function(event){
		if(scrollY>=50){
			menuNav.style.padding="15px 0px";
		}else{
			menuNav.style.padding="23px 0px";
		}
	})
}catch(error){
	console.log('навигационное мню не найдено')
}
